<?php

namespace Conecto\FeratelDsi\Dtos;

/**
 * Class representing NLMarketingCodeType
 *
 *
 * XSD Type: NLMarketingCodeType
 */
class NLMarketingCodeType
{
    /**
     * @var string $code
     */
    private $code = null;

    /**
     * @var int $order
     */
    private $order = null;

    /**
     * @var bool $global
     */
    private $global = null;

    /**
     * @var bool $multiSelect
     */
    private $multiSelect = null;

    /**
     * @var \Conecto\FeratelDsi\Dtos\TranslatableItemType[] $names
     */
    private $names = null;

    /**
     * @var \Conecto\FeratelDsi\Dtos\NLMarketingCodeValueType[] $codes
     */
    private $codes = null;

    public function __construct(string $code = null, int $order = null, bool $global = null, bool $multiSelect = null, array $names = null, array $codes = null)
    {
        $this->code = $code;
        $this->order = $order;
        $this->global = $global;
        $this->multiSelect = $multiSelect;
        $this->names = $names;
        $this->codes = $codes;
    }

    /**
     * Gets as code
     *
     * @return string
     */
    public function getCode()
    {
        return $this->code;
    }

    /**
     * Sets a new code
     *
     * @param string $code
     * @return self
     */
    public function setCode($code)
    {
        $this->code = $code;
        return $this;
    }

    /**
     * Gets as order
     *
     * @return int
     */
    public function getOrder()
    {
        return $this->order;
    }

    /**
     * Sets a new order
     *
     * @param int $order
     * @return self
     */
    public function setOrder($order)
    {
        $this->order = $order;
        return $this;
    }

    /**
     * Gets as global
     *
     * @return bool
     */
    public function getGlobal()
    {
        return $this->global;
    }

    /**
     * Sets a new global
     *
     * @param bool $global
     * @return self
     */
    public function setGlobal($global)
    {
        $this->global = $global;
        return $this;
    }

    /**
     * Gets as multiSelect
     *
     * @return bool
     */
    public function getMultiSelect()
    {
        return $this->multiSelect;
    }

    /**
     * Sets a new multiSelect
     *
     * @param bool $multiSelect
     * @return self
     */
    public function setMultiSelect($multiSelect)
    {
        $this->multiSelect = $multiSelect;
        return $this;
    }

    /**
     * Adds as translation
     *
     * @return self
     * @param \Conecto\FeratelDsi\Dtos\TranslatableItemType $translation
     */
    public function addToNames(\Conecto\FeratelDsi\Dtos\TranslatableItemType $translation)
    {
        $this->names[] = $translation;
        return $this;
    }

    /**
     * isset names
     *
     * @param int|string $index
     * @return bool
     */
    public function issetNames($index)
    {
        return isset($this->names[$index]);
    }

    /**
     * unset names
     *
     * @param int|string $index
     * @return void
     */
    public function unsetNames($index)
    {
        unset($this->names[$index]);
    }

    /**
     * Gets as names
     *
     * @return \Conecto\FeratelDsi\Dtos\TranslatableItemType[]
     */
    public function getNames()
    {
        return $this->names;
    }

    /**
     * Sets a new names
     *
     * @param \Conecto\FeratelDsi\Dtos\TranslatableItemType[] $names
     * @return self
     */
    public function setNames(array $names = null)
    {
        $this->names = $names;
        return $this;
    }

    /**
     * Adds as code
     *
     * @return self
     * @param \Conecto\FeratelDsi\Dtos\NLMarketingCodeValueType $code
     */
    public function addToCodes(\Conecto\FeratelDsi\Dtos\NLMarketingCodeValueType $code)
    {
        $this->codes[] = $code;
        return $this;
    }

    /**
     * isset codes
     *
     * @param int|string $index
     * @return bool
     */
    public function issetCodes($index)
    {
        return isset($this->codes[$index]);
    }

    /**
     * unset codes
     *
     * @param int|string $index
     * @return void
     */
    public function unsetCodes($index)
    {
        unset($this->codes[$index]);
    }

    /**
     * Gets as codes
     *
     * @return \Conecto\FeratelDsi\Dtos\NLMarketingCodeValueType[]
     */
    public function getCodes()
    {
        return $this->codes;
    }

    /**
     * Sets a new codes
     *
     * @param \Conecto\FeratelDsi\Dtos\NLMarketingCodeValueType[] $codes
     * @return self
     */
    public function setCodes(array $codes = null)
    {
        $this->codes = $codes;
        return $this;
    }
}

